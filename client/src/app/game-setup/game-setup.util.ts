...
export function generateGameSetup(config: GameConfig, retryCount = 0, startTime?: number): GameSetup {
	...
	const elapsed = Math.round(endTime - (startTime ?? 0));
	// console.log(`Board generated in ${attemptCounter.value} attempts after ${retryCount} retries. Elapsed time: ${elapsed} ms.`);
	return {
		board,
		ships: placedShips,
		config
