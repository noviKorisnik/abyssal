...
function generateShipPlacements(
	...
	if (shipTypeIndex >= shipTypes.length) {
		return true;
	}
	const shipType = shipTypes[shipTypeIndex];
	for (let count = 0; count < shipType.count; count++) {
		let placed = false;
		// Generate all possible positions and directions, then shuffle for randomness
			const directions: Array<'H' | 'V'> = shuffle(['H', 'V'] as Array<'H' | 'V'>);
			const positions: { row: number; col: number; dir: 'H' | 'V' }[] = [];
			for (const dir of directions) {
				const maxRow = dir === 'H' ? board.length - 1 : board.length - shipType.size;
				const maxCol = dir === 'H' ? board[0].length - shipType.size : board[0].length - 1;
		...
		for (const pos of positions) {
			...
			const cells = Array.from({ length: shipType.size }, (_, i) =>
			...
			if (isValidPlacement(board, cells)) {
				...
				placedShips.push({ id: shipId, type: shipType.name, size: shipType.size, cells });
				placed = true;
				break;
			}
		}
		if (!placed) {
			// Backtrack: remove previous ships of this type
			for (let i = placedShips.length - 1; i >= 0; i--) {
				if (placedShips[i].type === shipType.name) {
					removeShip(board, placedShips[i].cells);
	...
	// Next ship type
	if (generateShipPlacements(board, shipTypes, shipTypeIndex + 1, placedShips, shipIdCounter, attemptCounter)) {
		return true;
	}
	// Backtrack: remove all ships of this type
	for (let i = placedShips.length - 1; i >= 0; i--) {
		if (placedShips[i].type === shipType.name) {
			removeShip(board, placedShips[i].cells);
			placedShips.splice(i, 1);
...
export function generateGameSetup(config: GameConfig): GameSetup {
	...
	const success = generateShipPlacements(board, config.shipTypes, 0, placedShips, shipIdCounter, attemptCounter);
	if (!success) {
		throw new Error(`Failed to generate valid board setup after ${attemptCounter.value} attempts.`);
	}
	// Optionally, you can log or return attemptCounter.value for diagnostics
	return {
		board,
		ships: placedShips,
		config
