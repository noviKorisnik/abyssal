...
export function generateGameSetup(config: GameConfig, retryCount = 0): GameSetup {
	...
	if (!success || attemptCounter.value > MAX_ATTEMPTS) {
		if (retryCount < MAX_RETRIES) {
			return generateGameSetup(config, retryCount + 1);
		} else {
			throw new Error(`Failed to generate valid board setup after ${attemptCounter.value} attempts and ${MAX_RETRIES} retries.`);
		}
	}
	console.log(`Board generated in ${attemptCounter.value} attempts after ${retryCount} retries.`);
	return {
		board,
		ships: placedShips,
		config
